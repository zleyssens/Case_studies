...
% ── Settings ─────────────────────────────────────────────────────────
t_threshold = 3.13;
k_min       = 0;     % show all clusters including single voxels
...
for ci = 1:size(contrasts, 1)
    ...
    fprintf('Threshold: T = %.2f, p < 0.001 uncorrected, k >= %d\n', ...
        t_threshold, k_min)
    fprintf('Peak t = %.2f | Suprathreshold voxels = %d | Total clusters = %d\n', ...
        max(t_vol(:)), sum(bin_vol(:)), sum(clus_sizes >= max(k_min,1)))
    fprintf('========================================================\n')
    ...
    fprintf('%-5s %-5s %-8s %-22s %-40s\n', ...
        'Clus', 'k', 'T', 'MNI [x  y  z]', 'Brain region')
    fprintf('%s\n', repmat('-', 1, 85))
    ...
    for oi = 1:length(order)
        cc = order(oi);
        k  = clus_sizes(cc);
        if k < k_min && k_min > 0, continue; end
        n_shown = n_shown + 1;
